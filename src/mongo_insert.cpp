...
SinkFinalizeType MongoInsertOperator::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	// Invalidate cached schema for this collection so subsequent reads pick up the new data.
	try {
		...
		for (auto &attached : databases) {
			...
			if (catalog.GetCatalogType() == "mongo") {
				...
				mongo_catalog.InvalidateViewInfoCache(database_name, collection_name);
